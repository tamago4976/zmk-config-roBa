...
## `\` が `¥` や `§` になるとき（macOS 日本語配列）

macOS の **キーボード配列が「日本語」** のとき、US 向けの `\` キーコード（`BSLH` / `NUBS`）は `\` になりません（`¥` や `§` などになる）。

このキーマップでは **右下 = `jp_bslh` マクロ**（Option を押してから ¥ キーを送る）で `\` を送ります。`|` は **Space ホールド + 右下**（`LS(INT3)`）です。

代替: **システム設定 → キーボード → 入力ソース → 日本語 → 「¥」キーで入力する文字を `\` に変更** すると、OS 側で `\` に寄せられます。
